...
def generate_synthetic_lc(args, m0_data, amp_data, P_true, E_true, A_tmpl, Q_tmpl, 
    ...
    # synthetic magnitude
    mag_syn = np.zeros_like(t, dtype=float)
...
def synthetic_curve_analysis(args_full, m0_data, amp_data, tmpl_name, template,
    ...
    t, mag, emag, bmask, bands = args_full
    t0 = np.min(t) 
    ...
    mag_syn, amp_syn, scale = generate_synthetic_lc(args_full[:-1], m0_data, amp_data, P_true, E_true,
...
def monte_carlo_aliasing_analysis(sid, templates, mode=None,
    ...
    args_full = (t, mag, emag, bmask, bands)
    
    # mean / peak-to-peak amplitude
    m0_data, amp_data, _ = calculate_m0_amp(args_full[:-1]) 
